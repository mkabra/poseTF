...
def train(lblfile, nviews, name):
    for view in range(nviews):
        ...
        if conf.has_trx_file:
            multiResData.create_tf_record_from_lbl_with_trx(conf, False)
        else:
...
def classify(lbl_file, n_views, name, mov_file, trx_file, start_frame, end_frame, out_file, skip_rate):
    # print(mov_file)
...
def main(argv):
    ...
    if args.sub_name == 'train':
        ...
        classify(lbl_file, nviews, name, args.mov, args.trx, args.start_frame, args.end_frame, args.out_file, args.skip)
