...
# import PoseUNet
from PoseUMDN_dataset import PoseUMDN
import hdf5storage
import imageio
import multiResData
...
def convert_to_orig(base_locs, conf, cur_trx, trx_fnum_start, all_f, sz, nvalid, crop_loc):
    ...
    if conf.has_trx_file:
        ...
        for ii in range(nvalid):
            ...
            tt = -theta - math.pi / 2
            rot_mat = [[np.cos(tt), -np.sin(tt)], [np.sin(tt), np.cos(tt)]]
            curlocs = np.dot(base_locs[ii, :, :] - [hsz_p, hsz_p], rot_mat) + [x, y]
...
def create_conf(lbl_file, view, name, net_type='unet', cache_dir=None):
    ...
    proj_name = read_string(lbl['projname']) + '_view{}'.format(view)
    conf.view = view
    ...
    # If the project has trx file then we use the crop locs specified
    # by the user. If the project doesnt have trx files then we use the
    # crop size specified by user else use the whole frame.
    if conf.has_trx_file:
        ...
        if lbl['cropProjHasCrops'][0, 0] == 1:
            xlo, xhi, ylo, yhi = lbl[lbl[lbl['movieFilesAllCropInfo'][0, 0]]['roi'][view][0]].value[:, 0]
...
def create_leap_db(conf, split=False, split_file=None, use_cache=False):
    ...
    out_fns = [lambda data: train_data.append(data),
               lambda data: val_data.append(data)]
    if use_cache:
...
def create_deepcut_db(conf, split=False, split_file=None, use_cache=False):
    ...
    def deepcut_outfn(data, outdir, count, fis, save_data):
        ...
        locs = data[1]
        for bndx in range(conf.n_classes):
            fis[bndx].write('{}\t{}\t{}\n'.format(count[0], locs[bndx, 0], locs[bndx, 1]))
        mod_locs = np.insert(np.array(locs), 0, range(conf.n_classes), axis=1)
...
def get_trx_info(trx_file, conf, n_frames):
    if conf.has_trx_file:
        ...
        first_frames = np.array([x['firstframe'][0, 0] for x in trx]) - 1
        # for converting from 1 indexing to 0 indexing
    else:
        trx = [None, ]
        n_trx = 1
...
def get_trx_ids(trx_ids_in, n_trx, has_trx):
    if has_trx:
        if len(trx_ids_in) == 0:
...
def classify_movie(conf, pred_fn,
    ...
    def write_trk(pred_locs_in, n_done):
        ...
        cur_pred_locs = pred_locs_in.copy()
        ...
        hdf5storage.savemat(out_file,
                            {'pTrk': cur_pred_locs, 'pTrkTS': ts, 'expname': mov_file, 'pTrkiTgt': tgt,
...
def parse_args(argv):
    ...
    parser.add_argument('-model_file', dest='model_file',
                        help='Use this model file for prediction instead of the latest model file', default=None)
